...
class AdaptiveUNet(nn.Module):
    ...
    def __init__(
        self, in_channels: int = 2, out_channels: int = 2, acceleration: int = 4
    ):
        ...
        self.enc3 = self.conv_block(
            128, 256, kernel_size=5
        )  # Slightly smaller for deeper layers
        self.enc4 = self.conv_block(
            256, 512, kernel_size=3
        )  # Standard for high-level features
        ...
        self.final = nn.Conv2d(
            64, out_channels, kernel_size=main_kernel, padding=main_kernel // 2
        )
...
def visualize_predictions(model, data_module, num_samples=4):
    ...
    for i in range(min(num_samples, kspace_us.shape[0])):
        ...
        axes[i, 0].set_title("Input (Undersampled)")
        axes[i, 0].axis("off")
    ...
    print("📊 Visualization saved as 'mri_predictions.png'")
...
if __name__ == "__main__":
    ...
    if train:
        ...
        torch.save(
            {"model_state_dict": model.model.state_dict(), "hparams": model.hparams},
            "./adaptive-unet.pth",
        )
    ...
    data_module = MRIDataModule(
        dataset_path="./mri_dataset.h5", batch_size=16, num_workers=8, val_split=0.1
    )
