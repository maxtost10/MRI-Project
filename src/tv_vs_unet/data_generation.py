...
def create_undersampling_mask_with_acs(
    shape: Tuple[int, int], acceleration: int = 4, acs_lines: int = 24
) -> np.ndarray:
...
def generate_dataset(
    ...
    with h5py.File(save_path, "w") as f:
        ...
        # Create datasets
        for grp, num_samples, grp_name in [
            (train_grp, num_train, "train"),
            (test_grp, num_test, "test"),
        ]:
            # Preallocate arrays
            phantoms = grp.create_dataset(
                "phantoms", (num_samples, size, size), dtype="float32"
            )
            kspace_full = grp.create_dataset(
                "kspace_full", (num_samples, size, size), dtype="complex64"
            )
            kspace_undersampled = grp.create_dataset(
                "kspace_undersampled", (num_samples, size, size), dtype="complex64"
            )
            ...
            for i in tqdm(range(num_samples)):
                ...
                mask = create_undersampling_mask_with_acs(
                    kspace.shape, acceleration=acceleration, acs_lines=24
                )
                masks[i] = mask
