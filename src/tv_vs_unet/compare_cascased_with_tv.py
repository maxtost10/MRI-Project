...
def compare_cascaded_methods(
    ...
    """Compare Cascaded Model, K-space only U-Net, and TV
    denoising reconstruction methods.
    """
    # Load cascaded model manually from refinement_model.pth
    print("Loading cascaded model from refinement_model.pth...")
    refinement_checkpoint = torch.load(
        refinement_model_path, map_location=device, weights_only=False
    )

    # Create cascaded model manually
    cascaded_model = CascadedMRIReconstruction(
        kspace_model_path=kspace_model_path,
        learning_rate=refinement_checkpoint["hparams"].get("learning_rate", 1e-4),
        weight_decay=refinement_checkpoint["hparams"].get("weight_decay", 1e-5),
        lambda_l1=refinement_checkpoint["hparams"].get("lambda_l1", 1e-6),
    )

    # Load only the refinement model weights (k-space model is loaded internally)
    cascaded_model.refinement_model.load_state_dict(
        refinement_checkpoint["refinement_state_dict"]
    )
    ...
    with h5py.File(dataset_path, "r") as f:
        ...
        for idx in tqdm(
            ...
            if idx < 3:
                ...
                plt.tight_layout()
                plt.savefig(
                    f"plots_tv_cascaded_comparison/comparison_cascaded_sample_{idx}.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        "plots_tv_cascaded_comparison/performance_comparison_cascaded.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
if __name__ == "__main__":
    # Configuration - Updated to use refinement_model.pth
    DATASET_PATH = "./mri_dataset.h5"
    REFINEMENT_MODEL_PATH = "./refinement_model.pth"  # Updated path
    ...
    results = compare_cascaded_methods(
        dataset_path=DATASET_PATH,
        refinement_model_path=REFINEMENT_MODEL_PATH,  # Updated parameter name
        kspace_model_path=KSPACE_MODEL_PATH,
        num_samples=20,
    ...
    methods = ["Zero-filled", "K-space U-Net", "Cascaded Model", "TV-Denoise"]
    mean_psnr = {
        method: results[results["method"] == method]["PSNR"].mean()
        for method in methods
    }
