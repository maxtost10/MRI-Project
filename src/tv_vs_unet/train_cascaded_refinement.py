...
from train_unet import AdaptiveUNet, MRIDataset, MRIDataModule
...
class CascadedMRIReconstruction(pl.LightningModule):
    ...
    def _shared_step(self, batch, stage: str):
        ...
        reconstructed_kspace, reconstructed_image, refined_image = self.forward(kspace_us, mask)
        ...
        with torch.no_grad():
            ...
            psnr_unrefined = 10 * torch.log10(phantom_gt.max() ** 2 / (mse_unrefi + 1e-8))
...
def visualize_cascaded_results(model, data_module, num_samples=4):
    ...
    # Get predictions
    with torch.no_grad():
        reconstructed_kspace, reconstructed_image, refined_image = model(kspace_us, mask)
    ...
    for i in range(min(num_samples, kspace_us.shape[0])):
        ...
        print(f"Sample {i+1}:")
        print(f"  Before refinement - MSE: {mse_before:.6f}, PSNR: {psnr_before:.2f} dB")
...
def print_cascaded_summary(model, trainer):
    ...
    refinement_trainable = sum(p.numel() for p in model.refinement_model.parameters() if p.requires_grad)
...
if __name__ == "__main__":
    ...
    if TRAIN_CASCADED:
        ...
        model = CascadedMRIReconstruction.load_from_checkpoint(
            "checkpoints_cascaded/last.ckpt",
            kspace_model_path=KSPACE_MODEL_PATH
        )
